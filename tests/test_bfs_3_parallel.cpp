...
class TestBFS3Parallel : public Test {
   public:
    ...
    void run_assertions(std::vector<Ring> &result, size_t &bytes_sent_pre, size_t &bytes_sent_eval) override {
        if (conf.id == D) {
            size_t expected_pre = 4 * (16 * conf.size * conf.bits + 6 * 3 * conf.nodes + 8 * conf.size * 3 * conf.depth +
            ...
            size_t expected_eval = 4 * (12 * conf.size * conf.bits + 12 * 3 * conf.nodes + 4 * conf.size * 3 * conf.depth +
