...
tell application "Terminal"
    ...
    -- Window 1
    set w1 to do script "cd ~/CODE/GraphMPC/build/tests/test_mp && ./test_mp -p 1 --localhost"
    delay 0.2
    set bounds of front window to {0, 0, 500, 800} -- {left, top, right, bottom}

    -- Window 2
    set w2 to do script "cd ~/CODE/GraphMPC/build/tests/test_mp && ./test_mp -p 2 --localhost"
    delay 0.2
