...
static void send_vec(Party dest, std::shared_ptr<io::NetIOMP> network, size_t n_elems, std::vector<Ring> &data, const size_t BLOCK_SIZE) {
    ...
    for (size_t i = 0; i < n_msgs; i++) {
        ...
        network->send(dest, data_send_i.data(), sizeof(Ring) * BLOCK_SIZE);
    }
    ...
    network->send(dest, data_send_last.data(), sizeof(Ring) * last_msg_size);
}

static void recv_vec(Party src, std::shared_ptr<io::NetIOMP> network, std::vector<Ring> &buffer, const size_t BLOCK_SIZE) {
