...
class TestPiR : public Test {
   public:
    ...
    void run_assertions(std::vector<Ring> &result, [[maybe_unused]] size_t &bytes_sent_pre, [[maybe_unused]] size_t &bytes_sent_eval) override {
        if (conf.id == D) {
            [[maybe_unused]] size_t expected_pre = 4 * (16 * conf.size * conf.bits + 6 * conf.nodes * conf.nodes + 8 * conf.size * conf.nodes * conf.depth +
            ...
            [[maybe_unused]] size_t expected_eval = 4 * (12 * conf.size * conf.bits + 12 * conf.nodes * conf.nodes + 4 * conf.size * conf.nodes * conf.depth +
