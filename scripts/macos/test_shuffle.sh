...
tell application "Terminal"
    ...
    -- Window 1
    set w1 to do script "cd ~/CODE/GraphMPC/build/tests && ./test_shuffle -p 0 --localhost --vec-size 20"
    delay 0.2
    set bounds of front window to {0, 0, 500, 800} -- {left, top, right, bottom}

    -- Window 2
    set w2 to do script "cd ~/CODE/GraphMPC/build/tests && ./test_shuffle -p 1 --localhost --vec-size 20"
    delay 0.2
