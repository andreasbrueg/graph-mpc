...
int main(int argc, char **argv) {
    ...
    bpo::options_description cmdline("Benchmark the secure computation of the reach score."); // TODO name also others
    ...
    try {
        ...
        if (conf.id == D) {
            assert_comm_pre = 4 * (16 * conf.size * conf.bits + 27 * conf.size + 8 * conf.size * conf.depth + 6 * conf.nodes) +
            ...
            assert_comm_eval = 4 * (12 * conf.size * conf.bits + 17 * conf.size + 4 * conf.size * conf.depth + 12 * conf.nodes);
        }
