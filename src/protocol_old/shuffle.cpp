...
ShufflePre shuffle::get_shuffle(Party id, RandomGenerators &rngs, std::shared_ptr<NetworkInterface> network, size_t n, bool save) {
    ...
    switch (id) {
        case D: {
            ...
            network->add_send(P0, shared_secret_D0);
            network->add_send(P1, shared_secret_D1);
            return perm_share;
        }
        case P0: {
            shared_secret_D0 = network->read(D, n);
            return get_shuffle_compute(id, rngs, n, shared_secret_D0, shared_secret_D1, save);
        }
        case P1: {
            shared_secret_D1 = network->read(D, 2 * n);
            return get_shuffle_compute(id, rngs, n, shared_secret_D0, shared_secret_D1, save);
        }
    }
}
...
#pragma omp parallel for if (n > 10000)
    ...
    if (id == P0) {
        ...
        vec_A = network->recv_now(P1, n);
    } else {
        std::vector<Ring> data_recv = network->recv_now(P0, n);
