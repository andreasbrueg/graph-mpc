...
SecretSharedGraph share::random_share_graph(Party id, RandomGenerators &rngs, size_t n_bits, Graph &g) {
    ...
    SecretSharedGraph shared_graph(src_bits_shared, dst_bits_shared, isV_shared);
    shared_graph.dst_order_bits = dst_bits_shared;
...
std::vector<Ring> share::reveal_vec(Party id, std::shared_ptr<NetworkInterface> network, std::vector<Ring> &share) {
    ...
    switch (id) {
        case P0: {
            ...
            share_other = network->recv_now(P1, share.size());
        ...
        case P1: {
            ...
            share_other = network->recv_now(P0, share.size());
