   ...
   public:
    ...
    Graph create_graph(RandomGenerators &rngs) override {
        ...
        */
        ...
        g.add_list_entry(3, 3, 1, 1); // Starting point
    ...
    void run_assertions(std::vector<Ring> &result, size_t &bytes_sent_pre, size_t &bytes_sent_eval) override {
        if (conf.id == D) {
            ...
            // Assumes starting point v3 as specified in the input
            assert(result[0] == 1);
            assert(result[1] == 1);
            assert(result[2] == 1);
            assert(result[3] == 1);
            ...
            std::cout << "test_bfs passed." << std::endl;
        }
    }
};
...
int main(int argc, char **argv) {
    ...
    bpo::options_description cmdline("Test for the secure computation of BFS.");
