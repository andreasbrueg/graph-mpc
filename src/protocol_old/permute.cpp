...
/* ----- Preprocessing ----- */
void permute::switch_perm_preprocess(Party id, RandomGenerators &rngs, std::shared_ptr<NetworkInterface> network, size_t n, MPPreprocessing &preproc) {
    preproc.vtx_order_shuffle = shuffle::get_shuffle(id, rngs, network, n, true);
    preproc.src_order_shuffle = shuffle::get_shuffle(id, rngs, network, n, true);

    preproc.vtx_src_merge = shuffle::get_merged_shuffle(id, rngs, network, n, preproc.vtx_order_shuffle, preproc.src_order_shuffle);

    preproc.dst_order_shuffle = shuffle::get_shuffle(id, rngs, network, n, true);

    preproc.src_dst_merge = shuffle::get_merged_shuffle(id, rngs, network, n, preproc.src_order_shuffle, preproc.dst_order_shuffle);
    preproc.dst_vtx_merge = shuffle::get_merged_shuffle(id, rngs, network, n, preproc.dst_order_shuffle, preproc.vtx_order_shuffle);
}
...
std::vector<Ring> permute::apply_perm_evaluate(Party id, RandomGenerators &rngs, std::shared_ptr<NetworkInterface> network, size_t n, Permutation &perm,
    ...
    if (preproc.clear_shuffled_vtx_order.is_null()) {
        auto perm_shuffled = shuffle::shuffle(id, rngs, network, perm, preproc.vtx_order_shuffle, n);
        auto perm_opened = share::reveal_perm(id, network, perm_shuffled);
        preproc.clear_shuffled_vtx_order = perm_opened;
    }

    auto shuffled_input_share = shuffle::shuffle(id, rngs, network, input_share, preproc.vtx_order_shuffle, n);
    return preproc.clear_shuffled_vtx_order(shuffled_input_share);
}
...
std::vector<Ring> permute::switch_perm_evaluate(Party id, RandomGenerators &rngs, std::shared_ptr<NetworkInterface> network, size_t n, Permutation &p1,
                                                Permutation &p2, MPPreprocessing &preproc, std::vector<Ring> &input_share) {
    if (id == D) return std::vector<Ring>(n);

    switch (preproc.next_switch) {
        case vtx_src_1: {
            if (preproc.clear_shuffled_src_order.is_null()) {
                auto shuffled_p2 = shuffle::shuffle(id, rngs, network, p2, preproc.src_order_shuffle, n);
                preproc.clear_shuffled_src_order = share::reveal_perm(id, network, shuffled_p2);
            }

            preproc.next_switch = vtx_src_2;

            auto permuted_input = preproc.clear_shuffled_vtx_order.inverse()(input_share);
            auto permuted_input_shuffled = shuffle::shuffle(id, rngs, network, permuted_input, preproc.vtx_src_merge, n);
            return preproc.clear_shuffled_src_order(permuted_input_shuffled);
        }
        case vtx_src_2: {
            preproc.next_switch = src_dst;
        ...
        case src_dst: {
            if (preproc.clear_shuffled_dst_order.is_null()) {
                auto shuffled_p2 = shuffle::shuffle(id, rngs, network, p2, preproc.dst_order_shuffle, n);
                preproc.clear_shuffled_dst_order = share::reveal_perm(id, network, shuffled_p2);
            }

            preproc.next_switch = dst_vtx;

            auto permuted_input = preproc.clear_shuffled_src_order.inverse()(input_share);
            auto permuted_input_shuffled = shuffle::shuffle(id, rngs, network, permuted_input, preproc.src_dst_merge, n);
            return preproc.clear_shuffled_dst_order(permuted_input_shuffled);
        }
        case dst_vtx: {
            preproc.next_switch = vtx_src_1;

            auto permuted_input = preproc.clear_shuffled_dst_order.inverse()(input_share);
            auto permuted_input_shuffled = shuffle::shuffle(id, rngs, network, permuted_input, preproc.dst_vtx_merge, n);
            return preproc.clear_shuffled_vtx_order(permuted_input_shuffled);
        }
    }
}
