...
class TestPiM : public Test {
   public:
    ...
    void run_assertions(std::vector<Ring> &result, [[maybe_unused]] size_t &bytes_sent_pre, [[maybe_unused]] size_t &bytes_sent_eval) override {
        if (conf.id == D) {
            [[maybe_unused]] size_t expected_pre = 4 * (16 * conf.size * conf.bits + 27 * conf.size + 8 * conf.size * conf.depth) +
            ...
            [[maybe_unused]] size_t expected_eval = 4 * (12 * conf.size * conf.bits + 17 * conf.size + 4 * conf.size * conf.depth);
            assert(bytes_sent_pre == expected_pre);
            assert(bytes_sent_eval == expected_eval);
